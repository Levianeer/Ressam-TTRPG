...
The comparison worth keeping in mind: Ressam's Attributes now map almost exactly onto 5th Edition's ability *modifiers* (our 2 is a 14-15, our 3 a 16-17, our 5 a 20), and its standard array is nearly ours - **\+2, \+1, \+1, \+0** (5e's own six-value array, `\+2, \+2, \+1, \+1, \+0, \-1`, trimmed to four stats since Ressam has four Attributes to 5e's six - dropping one of its two \+2's and its \-1) against our **C**'s 2, 1, 1, 0. That makes 5e a usable yardstick for numbers of this shape, and the yardstick reads: **every ability-score prerequisite in 5e is exactly 13 - a \+1 modifier - and never higher**, with its later General feats using a flat *Level 4\+* gate instead of a bigger number. Ressam allows 2 on an ordinary Feat and 3 on a Prestige Feat, which is already a step and two steps past what 5e ever asks. See [[Feats|feats_overview]] for the rule as written - including how many levels those numbers actually take to reach, which depends on which priority letter you gave the Attribute in question, not just your Feats priority.
