...
## 67. The Reach/Tempo/Magic rework - four standalone playtest drafts adopted as the new mainline ruleset *(2026-09-20)*

User-directed, not `tempo_sim`-measured - a full mechanical overhaul, not a tuning pass, so this item records what changed and why the draft's numbers were trusted, not a win-rate delta.

**The source.** Four standalone files landed at the repo root - `combat_draft.md`, `equipment_draft.md`, `magic_draft.md`, `spells_draft.md` - a self-contained "playtest slice" adapted from Middle-earth Strategy Battle Game's Magical Powers for the magic half, and from HEMA-adjacent exchange-and-answer melee for the combat half. Each draft explicitly stripped things "so this slice actually tests itself": no leveling, no Feat-gating on magic schools, a narrow ~16-weapon list, no Arcane/Divine flavor split. The session's own decision was to adopt the drafts' *tested mechanics* as the new mainline rules wherever they diverged from the existing book on their own design merits, while reintegrating everything a draft cut purely as playtest scaffolding (leveling, Feat-gating, the full weapon/armor breadth, school flavor) back on top. That rule was applied consistently rather than re-litigated file by file.

**Combat (`core_rules.md`, `exchange.md`, `combat.md`, `positioning.md`, `rest_and_survival.md`):**

- **DC ladder recut a second time**, seven tiers down to four: `5/7/9/11`, Easy/Standard/Hard/Extreme - the same ladder a working's difficulty already used, so the two systems now share one table. The 17-point ceiling and the "12 over your modifier is a ruling, not a roll" rule are both unchanged; only the count of named rungs moved.
- **The Tempo Pool is no longer sized by an Attribute at all.** Every die is a flat `1d12` (STR no longer sizes the die; DEX no longer counts it); pool size is `4 + your weapon's (and shield's) Attacks modifier` instead. STR narrowed to melee damage only. DEX picked up **Initiative** (`5 + DEX`, moved off MIND) and the base **Shot DC** a target presents, in exchange for no longer sizing the pool - a straight swap of DEX's old job for a new one, not an addition.
- **Weapon Length (1-5) and the Edge mechanic are gone, replaced by three Reach bands** - Normal (Adjacent), Reach 1, Reach 2 - with no reach-mismatch bonus of any kind. Opportunity Attack triggers (entering vs. leaving) and the old Far Band/Length-5 pike's strike-past-an-intervening-square clause are both keyed off Reach now.
- **The Exchange itself flipped order: the defender rolls first**, setting a DC the attacker's investment must then beat, rather than both sides comparing simultaneously. Openings still trigger at margin 5, but each weapon now carries one **Signature Opening** (Riposte/Disarm/Sunder/Shove/Grapple) it reaches at margin 3 instead - replacing the old per-weapon Crit-turned-Opening-margin column (`-`/9/8) with a single named Opening per weapon rather than a uniform lowered threshold.
- **Surprise's old two-rung Caught Out/Ambushed system is gone, replaced by Awareness** - a pure GM fictional call, not a roll: an unaware target simply can't Parry the first attack against it, or eats a flat `-3` Shot DC row, and the ambush is spent after that one attack.
- **Death Clock, Coup de Grace, and Stabilization are gone, replaced by Downed.** A new Named/Unnamed split governs it: Unnamed creatures simply die at 0 Wounds, same as always; Named characters (PCs and any GM-named NPC) go Downed instead - Prone, 1-die Parry cap, any further Wound kills them, no ticking clock. **Patched Wounds is deleted as a resource** - healing (a Chirurgery check, a Healing Draught, a working) now restores real Wounds directly, at a cost: **every Wound healed costs 1 Trauma**, the new primary inflow into that track.
- **Trauma widened 0-5 to 0-20**, shared with the new magic system, and **lost its roll penalty entirely** - it now only gates Scars and kills outright at 20. The Wound Penalty (`-1` per missing Wound, Checks only, never a Tempo roll) is unchanged in shape and now carries the injury-cost job alone.
- **Scars reworked from damage-type-flavored narrative-only marks to Trauma-band-triggered mechanical ones** - Flinching, Haunted, Hollow, Wary, Hair-Trigger, Palsied, Steeled - tested with a new CHA-funded Resolve check when a fight ends above your prior Scar Line, and removed with a week of downtime plus a Chirurgery check rather than the old rare-and-expensive Cleric restoration.
- **Two wholly new systems**: **Fear/Morale** (a creature's Fear rating triggers a Will-funded resist the first time an enemy tries to close; separately, Nerve is a CHA check tested on set triggers - Unnamed creatures Break on a fail, Named characters take Trauma and never Break) and **Revelry & Leisure** (downtime in a town, the *only* way Trauma comes down - Rest heals bodies and gear, never minds, full stop). **Exerting** is new too: out of Tempo Dice but still need to Parry, spend a `1d6` and take 1 Trauma instead, capped at 1 per Parry.
- **Carrying capacity unified** from the two-pool Backpack Slots (`2 × STR`) / Bulky Capacity (`5 + Size`) split into one **Slots** pool, `6 + STR`, minimum 1 - equipped gear free, everything else 1 Slot flat.
- **Wound boxes rose**: Small/Medium/Large `3/4/5` -> `4/5/6`.

**Equipment (`weapons.md`, `armor.md`, `supplies.md`, `alchemy.md`):**

- Every weapon remapped from Length+Edge to Reach, gained an **Attacks** modifier (`+1` Light / `-1` Two-Handed / `+0` else - now mechanically load-bearing for Tempo Pool size, not flavor) and a **Signature Opening**.
- **Skills split further**: `Hafted & Polearms` -> `Hafted Weapons` (Normal Reach) + `Polearms` (Reach 1/2 - every polearm reclassified off Normal Reach, Quarterstaff and Spear included, which used to fight Adjacent). `Ranged` -> `Archery` (bows, crossbows) + `Firearms`. `Thrown` unchanged.
- **Armor-Piercing redefined**: a per-hit Dent/Rend reduction (bows AP 2, crossbows/pistols/arquebuses AP 3, heavy firearms 3-5) rather than "degrades armor by 2 durability instead of 1" - that old rule, and the matching War Maul special case, are both deleted, folded into the general **Blunt damage type now degrading armor 2x** on any landed hit (Unarmed is the one exception, at 1x).
- **Unarmored changed from a flat 2-Wound no-roll result to Dent 0 / Rend 5** - a real roll against a degenerate armor value, so a weak hit can still Turn Aside even with nothing worn. Deliberate, and flagged in the source draft as a departure from what shipped before it.
- **Armor field repair moved out of `armor.md` entirely**, folded into `rest_and_survival.md`'s Rest & Repair table (flat `+1` per Field Rest, `+1` or `+2` on a Long Rest with a Smithing check) - the old hourly Crafting-rank repair system is deleted. `armor.md`'s printed Dent/Rend numbers themselves were **deliberately left unchanged** - only the Unarmored mechanic and the repair cadence moved.
- Alchemy's Vital Oil recipes converted from granting Patched Wounds to healing real Wounds at the same Trauma cost as any other source; the Endurance Salve's old "removes Trauma via a Short Rest" effect was deleted outright rather than patched, since nothing is allowed to bypass Revelry & Leisure now.

**Magic (`magic_overview.md`, `laws_of_magic.md`, `minor_magic.md`, `magic_feats.md`):** the rank-1-5/`DC 5+rank`/flat-Feat-bonus casting roll the 2026-09-14 chassis had *just* built (see item history in `TODO.md`'s prior reset) was itself replaced, not merely retuned - the session judged the draft's MESBG-derived engine a better fit and adopted it wholesale over the just-finished system.

- **Mana renamed Will throughout** (`MIND × 3`, was `MIND × 2`).
- **MIND becomes a hard ceiling on dice committed** to a single cast or resist, not a roll modifier. Casting commits Will 1-per-die (paid before rolling, up to MIND dice), rolls that many `1d12`s and takes the **highest**, against a flat DC set by the working's tier - **Lesser/Common/Greater/Legendary at `5/7/9/11`**, sharing the same ladder `core_rules.md` now uses everywhere else. A miss still spends the Will.
- **Push** (spend Trauma, `+1` per point to the highest die, capped `+2`) and **Channel** (declare at the end of your prior turn, one die set to the working's difficulty exactly, Trauma-costed by tier, cast as your very next action) are both new - a defender can Push a resist the same way.
- **Resisting mirrors casting**: the target commits their own Will and rolls to tie or beat the caster's highest die - never a Ward, never a flat Feat bonus, for either side.
- **Damage workings ignore armor outright** (compared to Dent 0 / Rend 5 regardless of what's worn) and **carry no damage type** - the one edge a caster holds over a blade, paid for by rolling worse dice than a comparable weapon in every other respect.
- **`magic_feats.md` gained a fourth Unlock tier, Master** (Level 8, gating Legendary-difficulty workings), and **Focus Feats were redesigned around Signature Workings** (a working of your choice that costs less Will, and eventually Channels for free) rather than the flat `+1/+2/+3` roll bonus the new dice-count-based math has no room for.
...
## 71. magic_draft.md/spells_draft.md audit closed: Sanctuary mislabeled, and Learning Workings' costs traced to an old-Mana-Cost leftover *(2026-09-20)*

User-directed: finish the remaining, previously-unaudited territory of `magic_draft.md` (Casting/Resisting end to end, the Ten Schools framing, Learning Workings' Feat-tier shape, Designing Workings's Five Hard Limits/Held Will/area caps, the Free Dice suggested-trait list) plus a first full pass on `spells_draft.md`, which had never been checked at all. Read line-by-line against `magic_overview.md`, `laws_of_magic.md`, `minor_magic.md`, and `magic_feats.md`. Everything else holds - including every one of `spells_draft.md`'s fifteen worked examples against the Magic Damage rule (Ember Lash's 1d4 never reaching Rend 5, Chill Soul's 1d6 landing 2 Wounds on a 5-6, Banishment/Nature's Wrath landing 2 Wounds on almost any roll). Two real divergences found:

**Sanctuary mislabeled as a free-die trait.** `magic_overview.md`'s Hard Limit 1 read "matched in scope to a Sanctuary-style free-die trait." The draft's own Free Dice list defines Sanctuary as "total immunity to hostile workings for one target, one round" - explicitly not a die grant, unlike Steadfast/Bulwark/Rally which are. Fixed to "matched in scope to the Sanctuary trait," dropping the invented "free-die" qualifier before it could mislead a future Feat or trait built to that description.

**Learning Workings' Crown costs were the old Mana-Cost system wearing new labels.** `magic_overview.md`'s "Learning New Workings" paragraph priced starting workings at 50/100/150/200 Crown by tier. `magic_draft.md` never specifies a number at all - it only says learning one "costs downtime and money, and usually a trainer," and separately promises "character creation grants a token number of workings based on the character's highest Feat," a grant that had been dropped entirely rather than implemented anywhere. `git log` on the paragraph confirmed the mechanism's real origin: the pre-rework file's "50 Crown per Mana Cost" rule, carried forward with "Mana Cost" relabeled "tier" rather than replaced with the draft's own (unpriced) guidance - the same failure item 69 already found once in this file's Focus Feats. Resolution, by user call: keep the Crown numbers as an intentional house rule rather than reverting them, but stop leaving the token-workings grant unimplemented - added a **Workings Known at Character Creation** table (1/2/3/4 by highest Unlock Feat tier: Novice/Adept/Expert/Master) to `magic_overview.md`, on top of the existing buy-more-with-Crown option, and cross-referenced it from `character_creation.md` Step 7.

**Reviewed and left alone:** the Free Dice suggested-trait list (Steadfast/Bulwark/Rally/Sanctuary) was never meant as player-facing `core/` text - it's design fodder for whoever writes a free-die-granting trait later, and its absence from `magic_overview.md` isn't a gap. `laws_of_magic.md`'s "Mana Scarring"/"Mana Scar" does reintroduce the retired word "Mana" per `CLAUDE.md`'s terminology conventions, but was kept on review as a deliberate in-world proper noun (a physical mark from casting) rather than the retired resource name it happens to share a word with.

---

## 72. Magic Feats cut from Unlock+Focus+Combination to four pure-access Feats per school *(2026-09-20)*

User-directed redesign, on top of item 71's audit. The concept: **a Magic Feat is an access tax, not a toolkit** - unlike a Martial Feat, which *is* how a martial character's abilities actually expand, a caster's Feats should mostly just be the toll paid to cast at a given difficulty, with all real power growth coming from MIND, Will, and workings known instead (see `magic_overview.md`'s Learning Workings).

**The cut.** `magic_feats.md`'s per-school Focus tier (Novice: pick a Signature Working at one tier's discount; Adept: a Trauma discount on Channelling it; Expert: a second Signature Working; Master: Attunement) and its Combination Feats (cross-school workings, gated on holding the matching Unlock in every contributing school) are both deleted outright, not relocated or folded elsewhere - user call, matching the "cut entirely, no replacement" instruction. What remains per school is exactly the four Unlock Feats - Novice/Adept/Expert/Master - each one only gating that tier's casting difficulty, identical in shape across all ten schools with nothing school-specific left to write. The file's one worked example (Pyromancy) is gone too - with Focus and Combination cut, there's no longer any school-specific content a worked example would demonstrate that the generic template doesn't already state in full.

**Downstream fixes, same pass:** `feats_overview.md`'s Magic Feats blurb (dropped the Focus/Combination description, added the access-tax-vs-Martial-Feat framing), `magic_overview.md`'s Progression section (no longer claims Focus Feats spend its sideways-growth menu - reworded to note that menu is currently unclaimed by any Feat, open for a future racial trait or Prestige Feat instead), `templates/Home.md`'s Magic Feats line, and `CLAUDE.md`'s own description of `magic_feats.md`.

**Left deliberately stale:** `DESIGN_GUIDE.md`'s Magic Feat Guidelines section was already flagged stale pending the wider magic-system rewrite pass (see item 71's context and `TODO.md`) - this cut makes "Scale With Investment," "Damage Riders and Buffs Should Be Rare," and the "three shapes" framing describe a mechanic that no longer exists at all, on top of the Focus-shape drift already flagged. Folded into that same deferred rewrite rather than patched now.

---

## 73. Magic Feats written out for all ten schools, in the standard Feat block format *(2026-09-20)*

User-directed follow-up to item 72, same session: rather than leave the four-Feat shape as a single `<School>`-templated pattern, every one of the ten schools' four Feats is now written out concretely - `## <School>` sections matching `racial_feats.md`'s grouping, each Feat its own `### **<School>: <Tier>**` block with a one-line flavor sentence plus **Prerequisites:**/**Benefit:**, the same shape as `racial_feats.md`'s "Indomitable Spirit"/"Nine Lives" and the file's own prior Pyromancy worked example. Pyromancy's four flavor lines (already written for the old worked example, and already good) carried over unchanged; the other nine schools' lines are new, drawn from each school's own overview file (`arcane/*.md`, `divine/*.md` - the flavor paragraphs and Mana Scarring descriptions specifically, not the stale spell lists) so each school's Novice-to-Master progression escalates along that school's own established fictional idiom rather than a generic "you get better at magic" line repeated ten times. `magic_feats.md` is now a complete, finished chapter - forty Feats, nothing deferred.

**Superseded immediately by item 74, same session** - the user's original ask was for real Feat formatting, not forty separate Feats; this item's forty-Feat expansion over-read that request.

---

## 74. Correction: magic_feats.md reverted to a single four-Feat template, not forty per-school Feats

User clarified item 73 went further than intended: "I meant actually just four feats, to describe the mechanic." The ask was always for the *format* of a real Feat (a name, a flavor line, **Prerequisites:**/**Benefit:**) applied to the four tiers - not a separate instantiation per school. `magic_feats.md` reverted to one template: four Feat blocks (**Novice \[School\]** / **Adept \[School\]** / **Expert \[School\]** / **Master \[School\]**), each with a school-agnostic flavor line and real Prerequisites/Benefit text, `[School]` standing in for any of the ten with an explicit line saying so. The ten per-school flavor lines written for item 73 are discarded along with the forty-Feat structure - the four generic lines that replaced them (holding a working's shape, the working no longer fighting on the way out, telling it where it wanted to go, the working answering before the thought finishes) are close paraphrases of the original Pyromancy quartet, now genericized to fit any school rather than fire specifically. `CLAUDE.md` and `TODO.md` updated to describe the template shape again, not the forty-Feat one.

---

## 75. combat_draft.md audit: melee Tempo Dice were summed instead of taking the highest, plus four more confirmed bugs *(2026-09-20)*

User-caught, mid-design-conversation: while drafting `martial_feats.md`, a Feat's benefit was reasoned assuming melee Parries and Attacks sum every invested Tempo Die. The user flagged this as wrong and pointed at the source drafts. A direct re-read of `combat_draft.md` confirmed it: the Tempo Pool section states, unambiguously, "Take the **highest** die you rolled, not the sum," and The Exchange's own Defend/Attack steps repeat it ("`+ Weapon Skill + Guard` **on the highest die**" / "`+ Weapon Skill` **on the highest die**"). `exchange.md` had every one of these as "summed" instead - the resolution method for every Parry and Attack in the game, backwards from its own source, for who knows how long. This is the same failure shape as items 67-70, just found in the combat chapter instead of magic, and by the user rather than by a requested audit.

Given the severity, the user asked for a full line-by-line audit of `exchange.md`, `combat.md`, `positioning.md`, and `rest_and_survival.md` against `combat_draft.md` (and `equipment_draft.md` where relevant), not just the one fix. Most of the chapter held up - Openings, Shock, Fear, Nerve/Morale (including Steady the Line), the Damage Roll and Dent/Rend/Turned Aside math, Wound boxes and the Wound Penalty, most of Downed, Trauma's table and sources, Scars' effects table (transcribed word-for-word correctly), Unwinding, Awareness, Initiative, Opportunity Attack triggers, and the Shot DC table all check out. Five real bugs were found and fixed - see `TODO.md`'s now-closed "combat_draft.md audit" section for the full write-up of each:

1. **The sum-vs-highest bug itself** - fixed in `exchange.md`'s Exchange table (both steps) and the Defending section's Parry row, plus a new explicit statement at the top of the Tempo Pool section ("Every Tempo roll takes the highest die invested, not the sum") naming melee, a shot, and a cast together, specifically so this can't quietly drift back.
2. **Conditions table self-contradiction.** Prone's entry said "Parries at no penalty" and invented Advantage/Disadvantage-on-attack-rolls language found nowhere in the draft - directly contradicting `rest_and_survival.md`'s own Downed section a few lines down, which correctly caps a Downed (Prone) character's Parry at 1 die. Restrained carried the same invented language despite the draft explicitly saying "attacking... unaffected." Both restored to the draft's actual text. The rest of the Conditions table's D&D-shaped entries (Blinded, Frightened, Invisible, Poisoned, and others `combat_draft.md` never covers) still carry the same stale "attack rolls" phrasing and have no draft text to restore-from - flagged in `TODO.md` as a follow-up needing fresh design work, not touched this pass.
3. **Long Rest's bonus-healing DC** was 7 in the book, 9 in the draft - a full tier off. Fixed to 9.
4. **Polearms were missing "Ignores Guard" entirely**, despite `combat_draft.md` stating it as a blanket Reach-based rule and `equipment_draft.md` independently building an explicit Rapier-vs-Spear cost comparison around it ("one buys Reach and Ignores Guard, the other buys the knife through the visor - that's the trade"). Only the Chain Flail had it, for an unrelated, already-correctly-logged reason (item 2048 in this file). Added to all six Polearms' Properties, plus a rule statement in `weapons.md`'s Reach section and a rewritten Shields-section note that no longer implies the Chain Flail is the only weapon that has it.
5. **`exchange.md` kept a second, drifted copy of the Reach-to-weapon table** - missing Spear, Quarterstaff, and Lance from Reach 1, and wrongly describing Normal Reach as "every one-handed or Light melee weapon" (excludes Two-Handed Normal-Reach weapons like the Greatsword). `weapons.md`'s own table was already right. Deleted the duplicate from `exchange.md` in favor of a cross-reference - one copy to maintain instead of two that can disagree.

A sixth, smaller gap (not a draft contradiction so much as a dropped clause): the draft explicitly lists Riposte among what a Downed creature can't do ("can't make Opportunity Attacks, take Openings, or Riposte"); the book had dropped "or Riposte." Restored.

**Checked against `martial_feats.md`:** none of the seven Feats drafted earlier the same session hardcoded the broken math into their own wording (they invoke "Riposte," "free dice," and Armor-Piercing by name rather than restating the mechanic), so all seven hold up unchanged now that the underlying rule is fixed. Blade's Answer in particular reads better under the corrected model - roughly an Advantage-equivalent bonus on an already-earned Riposte, not the "maybe too strong, doubles your total" read that came from evaluating it against summed dice.

---

## 76. General, Martial, and Skill Feats written from scratch, then given a real prerequisite spread *(2026-09-20)*

User-directed, filling three of `core/feats/`'s empty stub headers with real content in one extended session, on top of item 75's combat audit.

**The shape, settled before any Feat was written.** The user's brief: Magic Feats are an access tax (see item 72); Martial Feats should be the opposite - a real toolkit expansion, the way a Martial Feat is *supposed* to differ from a Magic one. Feats should read as MTG-style keyword abilities: a plain flavor line naming a real historical or modern technique (no named school, master, or nation - `DESIGN_GUIDE.md`'s existing Real-World Grounding rule, which the first draft violated by naming German/Italian traditions directly before the user caught it and had them genericized), then a Benefit as one terse trigger/cost/effect clause. Where an existing term already covers the mechanic - an Opening, Armor-Piercing, Reach, Advantage, a free die, Steady the Line - the Benefit invokes it by name instead of restating it; a new keyword (Martial's **In a Bind**, an Exchange ending in a tie) only gets minted when nothing already fits. An earlier attempt to write out all ten schools' Magic Feats concretely (item 73) was overcorrected and reverted (item 74) after the user clarified they wanted the keyword *format* applied to a template, not forty literal Feats - the same lesson (build a small test batch, confirm direction, then scale) was applied here from the start: 2-3 draft Feats shown and refined before each batch's full 6-8 were written.

**Martial Feats (7):** Crossing Guard, Half-Sword Grip, Underhook, Set the Line, Concussive Blow, Double-Shotted, Blade's Answer - one per weapon-Skill niche (a generic STR-melee bind trick, Two-Handed Blades, Daggers & Wrestling/grappling, Polearms, Hafted Weapons, Firearms, Fencing Blades), deliberately not covering Cleaving Blades/Archery/Thrown this pass. Caught and fixed during drafting: Half-Sword Grip's original "Reach Normal" downside was a no-op (Two-Handed Blades weapons are already Normal Reach), replaced with "no Signature Opening"; Concussive Blow's "Your Blunt hits" was too broad (would've caught a Shield Bash or a Blunt-mode Quarterstaff), narrowed to "Blunt hits with a Hafted weapon."

**General Feats (8):** Watchful, Second Wind, Tough, Command Presence, Hardened, Iron Nerve, Combat Medic, Deep Reserves - broad utility rather than weapon-grouped. Hardened's prerequisite ("Hold at least 1 Scar") is a real state gate, not a chargen-time build choice - flagged to the user rather than shipped silently, and resolved with a second route: taken at character creation, it costs starting play with 1 Wound already marked off instead, so the "already lived through worse" flavor holds either way.

**Skill Feats (7):** Tradecraft, Tracker, Apothecary, Armorer's Eye, Read the Room, Second-in-Command, Horse Sense - one per non-combat trade. Tradecraft specifically fulfills a promise `attributes_and_skills.md` already made ("Specializing in one mode [of Subterfuge]... is a Feat choice, not a Skill investment"). Apothecary and Armorer's Eye were checked against `alchemy.md`'s actual Batch Size formula and `rest_and_survival.md`'s Long Rest repair check before writing their Benefits, rather than inventing new numbers. Second-in-Command's Cohort-Level buff was checked against `leadership.md`'s existing two-Cohort penalty for a stacking exploit before shipping.

**Two real findings along the way, not planned:**

- **Wards, re-confirmed live.** The user asked whether Wards were eliminated by `combat_draft.md`/`magic_draft.md`, since neither draft mentions the term. Checked directly: `core_rules.md`'s Ward system is intact and already reconciled with the reworks (it explicitly excludes spellcasting, "never targets a Ward," matching Fear/hostile-magic resistance's real move to the Will-based resist system) - the drafts simply never needed to exercise traps/poison/falls in their narrow playtest slice. Ward stays real; logged as a candidate for its own future look in `TODO.md` anyway, since it's the one defensive roll left on the old flat-check shape while melee, shots, and casting all moved to invest-dice-take-highest.
- **A pre-existing bestiary conflict.** `universal.md`'s Knight and `mythical.md`'s dragon already cited a "Second Wind" and "Tough" Feat by name with specific mechanics, discovered only because checking the Ward question led through `attributes_and_skills.md`'s cross-references into the bestiary. The already-written Second Wind (Advantage on STR/DEX Wards) differed from the bestiary's older flat `+3`/reroll version, and "Tough" didn't exist yet at all. User call: keep the new Second Wind, add Tough (`+1` Wounds, no prerequisite) to match what the bestiary assumed, and update both bestiary lines to the final wording instead of rewriting the Feat.

**The prerequisite spread, added last.** The user flagged that Ressam's Attribute scale mirrors 5e's ability *modifiers* closely enough that a flat `+2`-ranks-or-Attribute-2 ceiling across all 22 Feats - which is what every one of them shipped with - reads as no curve at all, just one uniform bar. `feats_overview.md` already had the tool needed to fix this without inventing a new rule: a bare Attribute prerequisite caps at 2 for an ordinary Feat, but a Skill-rank prerequisite is separately sanctioned up to `+3` ("the intended ceiling for 'this is a mastery pick,' a level gate, not an Attribute gate"). Retiered across all three files: `+1` rank/Attribute 1 for a broadly-available starter pick, `+2`/Attribute 2 for the default, `+3` ranks in a Skill for mastery - routed through a Skill rather than a bare Attribute, since an ordinary Feat still can't ask for Attribute 3 directly. Five Feats moved to `+3` also got their Benefit strengthened to match the harder ask, not just a bigger number: Set the Line's Armor-Piercing 3 now applies to every attack with a Reach 1/2 Polearm, not just Opportunity Attacks; Blade's Answer's free dice went from two to three; Command Presence's Steady the Line extension lost its range cap entirely; Combat Medic's fast revival now also heals 1 extra Wound. `feats_overview.md` gained a new paragraph documenting the three-rung shape so it isn't re-derived from scratch next time a Feat gets written.

---

## 77. Prestige Feats removed, not patched - the pre-Ressam D&D homebrew content finally hit a wall *(2026-09-21)*

User-directed audit of `prestige_feats.md`'s seven Feats (Iron Path, Final Strike, Blood-Rule, Mirage of Light, Bulwark of Mind, Fallen Wing, Soul-Bond) against current rules, continuing the item-76 Feats pass. Unlike the three empty stubs filled in last item, this file already had real content - built for the D&D homebrew Ressam grew out of, per the user's own framing going in ("massively game warping, built originally for the old DND homebrew that inspired Ressam").

**The audit found three tiers of problem, not one:**

1. **Straightforward terminology drift** - "Mana" instead of "Will" (four times in Blood-Rule, once in Bulwark of Mind), a stale "Rolling Magic" section reference (now "Casting"), an incomplete Openings list in Iron Path (missing Sunder and Shove, inventing a "called shot" that isn't a term here).
2. **Numeric systems with no current mapping** - "the spell's rank ÷ 2" (Blood-Rule) and "1d8 × the spell's rank" (Fallen Wing) both assume a numeric spell-rank system the current Lesser/Common/Greater/Legendary tiers replaced; converting them requires a real design choice (the natural fit is reusing Channelling's existing tier-cost scale), not a find-and-replace.
3. **Content with no current equivalent to fall back on at all** - Final Strike's "you can no longer trade places with another PC" cites a mechanic that doesn't appear anywhere else in the entire book; nobody could say what it currently does. Soul-Bond cited `core_rules.md`'s Skill Check Formula (out-of-combat Checks) to describe the Exchange (the actual combat resolution), the wrong rule entirely - and separately claimed the Feat swaps out "STR or DEX" for damage, when DEX never funded melee damage even pre-rework, so that clause was never accurate at any point in this book's history.

Blood-Rule and Fallen Wing additionally price themselves against `invocation.md`'s Wound Loss Rule specifically - one of the still-unconverted spell files on `TODO.md`'s deferred list - so fixing their numbers now would mean guessing at a rule that pass is going to rewrite anyway.

**Resolution, by user call: remove rather than patch piecemeal.** `prestige_feats.md` kept its framework (Level 5 gate, one Prestige Feat max, narrative ritual - none of it referenced anything stale) and lost all seven Feats, back to the same "empty stub, real rebuild deferred" shape `general_feats.md`/`martial_feats.md`/`skill_feats.md` were in before item 76. One live cross-reference broke and was fixed: `invocation.md` named "the Blood-Rule Prestige Feat" by name as an example of a Mana-to-Wounds conversion; genericized to "some other effect" since there's nothing left to point at. Every other mention of Prestige Feats across the book - `feats_overview.md`'s prerequisite-ceiling rule, the Level 5 note in `progression_&_rewards.md`'s advancement table, `magic_feats.md`, `magic_overview.md`, `DESIGN_GUIDE.md`'s 5e-mirror comparison, both wiki nav templates, the bestiary-npc skill's effective-level note - is framework-level language that stays true with the category empty, and none of it needed touching. Logged in `TODO.md` as its own item rather than folded into the general spell-conversion backlog, since a rebuild should start from the current Exchange/magic rules rather than porting the old mechanics line-by-line - Iron Path's total-Parry-and-Opening removal and Final Strike's turn-order inversion are worth keeping as *design targets*, just not as the specific implementations sitting in git history.

---

## 78. racial_feats.md audit, and the same STR/DEX damage slip found in two race Features *(2026-09-21)*

User-directed audit of `racial_feats.md`'s two entries (Indomitable Spirit/Human, Nine Lives/Feliid) against current rules, continuing the Feats pass from items 76-77. Much smaller find than Prestige Feats - the file was already close to current.

**Indomitable Spirit's scope didn't match its own exception clause.** The Benefit spends points to bump "an Attribute or Skill roll" - a Skill Check or Ward Check, in current terms - but then explicitly carves out firearm Misfire as something it can't affect. Misfire only ever triggers off a Tempo Die invested in a shot, which the stated scope never touches in the first place - a leftover from some earlier, wider version of the Feat (plausibly one that covered Tempo rolls generally) that never got cleaned up when the scope narrowed. User call: keep the narrow scope, reword to the exact current terms ("a Skill Check or Ward Check"), delete the now-pointless Misfire clause rather than widening the Feat to justify keeping it. Nine Lives needed nothing - it already maps cleanly onto the current Downed/death trigger.

**Found in passing: the same STR-vs-DEX damage bug in two race Features.** Checking `racial_feats.md` against its parent race files for overlap turned up `feliids.md`'s Claw ("1d4+DEX slashing... (finesse)") and `skeggs.md`'s Bristling Spines ("1d4+DEX piercing") both keying a damage roll off DEX - but melee damage has been STR-only without exception since the Reach/Tempo rework, finesse weapons included (`combat.md`). Both fixed to `+STR`; Feliid's `(finesse)` tag dropped too, since "Finesse" isn't a defined weapon property anywhere in the current `weapons.md` - it only ever existed to prop up the DEX swap that's now gone. A follow-up grep across every race file, the full bestiary, and `core/equipment/` found no third instance. Notable because `TODO.md` and `CLAUDE.md` both currently claim the race files already "reflect current combat/equipment terminology" - this pair evidently slipped through whatever pass made that claim, so it's not a fully safe assumption to keep leaning on without spot-checks.

---

## 79. feats_overview.md read-through finds the Attribute-progression math was wrong for two of five priorities *(2026-09-21)*

User-directed full read-through of `feats_overview.md`, closing out the last unchecked file in `core/feats/` after items 76-78. Most of it held up - the Feat-grant schedule, the Skill-Rank-implies-Attribute rule, the six-Feats-at-C-priority count all checked out against `progression_&_rewards.md` and `character_creation.md`. One real error, load-bearing enough that it had been cited as justification throughout this session's whole Feats pass.

**The error:** "Outside an A priority, an Attribute of 2 is where you start" is true for **B** (`2,2,1,0`) and **C** (`2,1,1,0`) but false for **D** (`1,1,1,0`) and **E** (`1,1,0,0`) - neither array has a 2 anywhere in it. D/E don't reach 2 until Level 4's Attribute bump, 3 until Level 8, and never reach 5 through leveling alone (three bumps on a starting 1 caps at 4 by Level 12). Two dependent claims inherited the error: "a 3 is in hand at Level 4" (only true for B/C; D/E need Level 8) and Prestige's "asking for 4 would put the whole category out of reach until Level 8 for four of the five priority letters" (wrong for D/E in a second way too - they don't reach 4 until Level 12, not 8).

**Fixed with a table**, not more prose, matching this repo's own table-over-prose convention for exactly this kind of comparative data: priority letter by Level 1/4/8/12, showing exactly when each reaches each Attribute value. Per the user's explicit ask ("make sure this does land roughly around D&D's attribute modifiers for balance"), each cell also shows its 5e-modifier equivalent, since `DESIGN_GUIDE.md`'s 5e mirror is the established sanity check for these numbers and the user wanted it checked, not just the arithmetic fixed in isolation.

**Checking that sanity check found a second, independent bug.** `DESIGN_GUIDE.md` compared 5e's six-value standard array (`+2,+2,+1,+1,+0,-1`) against "our C's 2, 2, 1, 1, 0, 0" - six values, and the wrong ones besides. Ressam has had four Attributes (STR/DEX/MIND/CHA) for a while now; the real C array is `2, 1, 1, 0`, four values. Leftover from before some earlier consolidation from six Attributes down to four that never propagated into this specific comparison. Fixed by trimming 5e's array to four stats (dropping one of its two `+2`s and its `-1`) to actually match Ressam's four-Attribute shape.

**A third error caught mid-fix, in the fix's own first draft.** Rewriting the "no Feat asks for more than +3 ranks in a Skill" bullet, the draft claimed Level 8 (where a D/E Attribute reaches 3) is "a level a Feat is granted at anyway," extending the original text's true claim about Level 4. Checked against `progression_&_rewards.md` before shipping: Feats land at 4, 6, 9, and 12; Attribute bumps land at 4, 8, and 12 - a separate schedule that only shares Levels 4 and 12 with the Feat-grant one. Level 8 grants no Feat at all. Reworded to say what's actually true - the stat is sitting there waiting by Level 9, the next slot that opens, not that Level 8 itself hands out anything.

**Nothing about the actual rule changed** - 2 for an ordinary Feat, `+3` ranks for a mastery pick, 3 for Prestige are all unaffected, and still land exactly where `DESIGN_GUIDE.md` intends (a full step, then two steps, past 5e's own +1-modifier ceiling). This was a correctness pass on the justification text alone; none of the 22 Feats written in items 76-78 needed any change. Also tightened the Martial Feats category blurb, which never stated the toolkit-expansion framing the Magic Feats blurb explicitly contrasts itself against - it was pointing at a line the Martial entry itself never said.
