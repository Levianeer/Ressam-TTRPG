...
**Note:** the playtested win-rate below is carried over from a prior simulation as a starting reference point, not a re-run number - see `TODO.md` on the state of `tools/`.
...
| :---: | :---: | :---: | :---: |
| 6 | Draconic scale (9 / 14) | 4 dice at `1d12` | 7 |

**Wards:** STR 10, DEX 7, MIND 8

**Parry:** `1d12 (per die) + 5`, scale and bulk - a Wyrm doesn't carry a weapon or shield, but its Daggers & Wrestling rank still answers a blow.

**Movement:** 40 ft - legless, but no less mobile for it; a serpentine glide rather than a walk.

**Natural Weapons:** Bite and Claw (2d8 Piercing/Slashing, Reach 1, `+0` Attacks).

**Damage:** 2d8 \+ 5 (STR).

**Size:** Huge (6 Wounds \= baseline 5 \+ Tough; see [[Carrying Capacity|rest_and_survival]] on scaling the baseline past Large for GM-statted creatures).

**Mythic Initiative (4):** Rolls initiative 4 times (normal, \-2, \-4, \-6); takes a full turn on each count. See [[Mythic Initiative|bestiary_overview]] - its Tempo Pool refills in full on its first count and regains 1 die on each count after, and end-of-turn effects (Bleed, Frightened saves, and the like) trigger on every one of its turns, not once per round.

**Dragonfire Breath (1/round, any one of its turns):** A 60 x 5 ft line of fire. Every creature in the line: DEX Ward (DC 11, Extreme) or take 2d10 \+ STR Fire damage - a success halves the damage instead of negating it outright. Once per round total, per Mythic Initiative's Repetition rule ([[Mythic Initiative|bestiary_overview]]) - not once per turn.
...
**In Combat:** A Wyrm doesn't maneuver for advantage - it opens with Dragonfire Breath on whichever turn is most convenient, then spends its other three turns each round on Bite and Claw against whoever's closest, its scale and 6 Wounds absorbing the return fire. Playtested solo at 50.7% monster-favored against a Level 1 Balanced 4-PC party (3,000 trials, avg 1.6 rounds) - a genuine coin flip, and it should feel that way at the table: dangerous, not unwinnable. Keep the breath's damage modest: a guaranteed, no-roll-to-cast, whole-party hit is worth far more than the same dice on a single target - a normal working prices that tradeoff into its own cast risk and Will cost, and a monster ability that just always works skips that cost entirely, so the dice size has to do that job instead.