## Slots

Carrying capacity is tracked in **Slots**, not weight. Check your Slots when you decide what to carry \- not on every purchase or pickup.

**Slots \= 2 × STR**, minimum 0 \- a negative STR floors here rather than giving you negative capacity, so a STR 0 or lower character simply carries nothing beyond what they are wearing and holding.

Size modifies your Slot total, rounding down:

|  Size  | Slots Modifier | Space (Squares) | Length |
|:------:|:--------------:|:--------------:|:--------------:|
| Small  |     × 0.5      |   5 ft (1×1)   |    1     |
| Medium |      × 1       |   5 ft (1×1)   |    1     |
| Large  |      × 2       |  10 ft (2×2)   |    2    |
|  Huge  |      × 4       |  15 ft (3×3)   |     3     |

**Space:** How much room a creature occupies, in grid squares (see [[Battlemap \& Positioning|positioning]]).
**Length:** How far its unarmed melee reach extends, on the same 1-5 scale a weapon uses (see [[Weapon Length|exchange]]). A Small or Medium creature's fists sit at **Length 1** with the daggers, the same as any character's; scale carries a Large creature to **Length 2** and a Huge one to **Length 3** - all three fight Adjacent regardless, the number only feeding the Edge and Sunder/Disarm split. **Nothing natural reaches Length 4 or 5** - the pike's strike-past-an-intervening-creature clause belongs to it and to nothing else, a giant's arms included.

**Encumbered (items exceed Slots):** Speed drops to 5 ft; disadvantage on physical checks; cannot rest.

**Push/Drag/Lift:** Up to 2× your Slot capacity in equivalent bulk, but speed drops to 5 ft.

### **Slot Costs**

- **Tiny items** (trinkets, coins, small tools): 3 identical or similar items per Slot. A Belt Pouch holds 3 Tiny items for free.
- **Standard items** (most weapons, tools, clothing): 1 Slot.
- **Heavy items** (armor, larger tools): 2 Slots.
- **Bulky items** (tents, large furniture): 3 Slots.
- Anything larger: 1 Slot per 10 lb-equivalent of bulk, rounded up.
- **Coins:** 100 coins \= 1 Slot.
- **Bundles:** Torches, Candles, Rations, and Oil flasks are each sold and carried in bundles of 3 \= 1 Slot.

A **Backpack** costs no Slots itself, but is required to use your full Slot count \- without one, you can only carry STR Slots worth of gear. Worn armor and an equipped weapon or shield cost no Slots; the Slots value on an item only applies while it's carried as a spare.

Most ammunition comes in backs of a specifc number of shots, the packs themselves usually have a set slot cost. 

---

## Resting

**Note:** Permanent Wounds do not heal on this ladder \- see [[Wound Recovery|wounds_and_survival]]. Patched Wounds do, below.

### **Short Rest (2 Hours)**

- Patched Wounds become at least 1, never lower than what you already have  
- Regain 1 Mana. Divine casters must also spend part of this Rest in genuine prayer to receive it (see [[Magic Overview|magic_overview]]'s Devotion Required rule) - Arcane needs nothing beyond completing the Rest  
- Trauma: Chirurgery DC 12 to remove 1 level  
- Can use: Crafting, Chirurgery, Scribing, Prayer etc

### **Field Rest (8 Hours)**

- Patched Wounds become at least 2, never lower than what you already have  
- Regain Mana equal to **MIND**. Divine casters must also spend part of this Rest in genuine prayer to receive it (see [[Magic Overview|magic_overview]]'s Devotion Required rule) - Arcane needs nothing beyond completing the Rest  
- Trauma: Chirurgery DC 12 to remove 2 levels  
- Can use: Crafting, Chirurgery, Scribing, Prayer etc  
- **Requires:** Shelter (a tent, lean-to, or similar \- not a permanent structure), watch rotation, defensible position, 1 ration per character (trail food)

### **Long Rest (24 Hours)**

- Patched Wounds become at least your missing Wounds (fully topped off)  
- Regain Mana equal to **MIND × 3**, up to your Maximum Mana. Divine casters must also spend part of this Rest in genuine prayer to receive it (see [[Magic Overview|magic_overview]]'s Devotion Required rule) - Arcane needs nothing beyond completing the Rest  
- Trauma: Chirurgery DC 12 to remove 3 levels  
- Can use: Crafting, Chirurgery, Scribing, Prayer etc  
- **Requires:** **Good Shelter** (a permanent, secure structure \- inn, barracks, temple, private home, etc. \- not a tent or camp), **Good Food** (a proper hot meal, Meal common or better \- see [[Supplies|supplies]], not trail rations), civilization with security

**Note:** Field Rests and Long Rests provide no benefits if their shelter and food requirements aren't met. Field Rest and Long Rest take the same 8 hours \- what separates them is the quality of shelter and food required, not the time spent.
